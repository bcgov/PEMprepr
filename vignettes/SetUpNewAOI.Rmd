...
library(bcmaps)
trim <- cded_raster(aoi)
# Still to do: convert to crs and align
...
```{r}
# align the DEM file with a 25m template raster
dem_25 <- align_raster(iraster = dem, rtemplate = r25)
...
create_covariates(dtm = dem_25,           ## raster created above
                  SAGApath = "C:/SAGA/saga-7.7.0_x64/", ## Where SAGA GIS is installed
                  output = fid$cov_dir_1020[2], ## from the setup_folders above
                  layers = "filled_sinks")        ## test one is working 

# run all covariates 
create_covariates(dtm = dem_25,           ## raster created above
                  SAGApath = "C:/SAGA/saga-7.7.0_x64/", ## Where SAGA GIS is installed
                  output = fid$cov_dir_1020[2], ## from the setup_folders above
                  layers = c( "steepestslope", "upslopearea"))       ## use all or one of the above
